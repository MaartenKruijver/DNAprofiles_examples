...
cdf.l <- dist.pair.cdf(dists.product.pair(si.fs.dist,appr=TRUE,appr.method=1))  # `round' to lower values
cdf.u <- dist.pair.cdf(dists.product.pair(si.fs.dist,appr=TRUE,appr.method=2))  # `round' to higher values
